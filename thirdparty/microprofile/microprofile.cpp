...
//		**(instrumented only option in timers) Instrumented functions in a
//separate view 		*create dirty list of microprofile functions to never instr
...
void
MicroProfileSleep(uint32_t nMs);
template<typename T>
T
MicroProfileMin(T a, T b);
template<typename T>
T
MicroProfileMax(T a, T b);
template<typename T>
T
MicroProfileClamp(T a, T min_, T max_);
int64_t
MicroProfileMsToTick(float fMs, int64_t nTicksPerSecond);
float
MicroProfileTickToMsMultiplier(int64_t nTicksPerSecond);
uint64_t
MicroProfileLogGetType(MicroProfileLogEntry Index);
uint64_t
MicroProfileLogGetTimerIndex(MicroProfileLogEntry Index);
MicroProfileLogEntry
MicroProfileMakeLogIndex(uint64_t nBegin,
                         MicroProfileToken nToken,
                         int64_t nTick);
int64_t
MicroProfileLogTickDifference(MicroProfileLogEntry Start,
                              MicroProfileLogEntry End);
int64_t
MicroProfileLogSetTick(MicroProfileLogEntry e, int64_t nTick);
uint16_t
MicroProfileGetTimerIndex(MicroProfileToken t);
uint32_t
MicroProfileGetGroupMask(MicroProfileToken t);
MicroProfileToken
MicroProfileMakeToken(uint64_t nGroupMask,
                      uint32_t nGroupIndex,
                      uint16_t nTimer);
bool
MicroProfileAnyGroupActive();

//////////////////////////////////////////////////////////////////////////
// platform IMPL
void*
MicroProfileAllocInternal(size_t nSize, size_t nAlign);
void
MicroProfileFreeInternal(void* pPtr);
void*
MicroProfileReallocInternal(void* pPtr, size_t nSize);

void*
MicroProfileAllocAligned(size_t nSize, size_t nAlign);
void
MicroProfileFreeAligned(void* pMem);
...
inline int64_t
MicroProfileTicksPerSecondCpu_()
{
...
inline uint64_t
MicroProfileGetCurrentThreadId()
{
  uint64_t tid;
...
void
MicroProfileSymbolQueryFunctions(MpSocket Connection, const char* pFilter);
void
MicroProfileInstrumentFunction(void* pFunction,
                               const char* pModuleName,
                               const char* pFunctionName,
                               uint32_t nColor);
bool
MicroProfileSymbolInitialize(bool bStartLoad, const char* pModuleName = 0);
MicroProfileSymbolDesc*
MicroProfileSymbolFindFuction(void* pAddress);
void
MicroProfileInstrumentFunctionsCalled(void* pFunction,
                                      const char* pModuleName,
                                      const char* pFunctionName);
void
MicroProfileSymbolQuerySendResult(MpSocket Connection);
void
MicroProfileSymbolSendFunctionNames(MpSocket Connection);
void
MicroProfileSymbolSendErrors(MpSocket Connection);
const char*
MicroProfileSymbolModuleGetString(uint32_t nIndex);
void
MicroProfileInstrumentWithoutSymbols(const char** pModules,
                                     const char** pSymbols,
...
struct MicroProfile
{
  ...
  int64_t
    nCounterHistory[MICROPROFILE_GRAPH_HISTORY]
                   [MICROPROFILE_MAX_COUNTERS]; // flipped to make swapping
                                                // cheap, drawing more expensive.
...
inline MicroProfileLogEntry
MicroProfileMakeLogPayload(const char* pData, uint64_t nSize)
{
  MP_ASSERT(nSize < 8);
...
inline void
MicroProfileGetLogPayload(uint64_t nMessage, char* pData)
{
  MicroProfilePayloadPack P;
  P.LogEntry = nMessage;
...
inline int64_t
MicroProfileLogTickMax(MicroProfileLogEntry A, MicroProfileLogEntry B)
{
...
inline int64_t
MicroProfileLogTickMin(MicroProfileLogEntry A, MicroProfileLogEntry B)
{
  int64_t Diff = MicroProfileLogTickDifference(A, B);
  if (Diff < 0) {
    return B;
...
inline int64_t
MicroProfileLogTickClamp(uint64_t T, uint64_t min, uint64_t max)
{
  return MicroProfileLogTickMin(MicroProfileLogTickMax(T, min), max);
}

inline int64_t
MicroProfileLogGetTick(MicroProfileLogEntry e)
{
...
inline uint32_t
MicroProfileGetGroupMaskIndex(MicroProfileToken t)
{
  return (uint32_t)(t >> 48);
}

inline MicroProfileToken
MicroProfileMakeToken(uint32_t nGroupMask,
...
float
MicroProfileTickToMsMultiplierGpu()
{
  return MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondGpu());
}
uint16_t
MicroProfileGetGroupIndex(MicroProfileToken t)
{
...
typedef pthread_t MicroProfileThread;
void
MicroProfileThreadStart(MicroProfileThread* pThread,
...
void
MicroProfileThreadJoin(MicroProfileThread* pThread)
{
...
static void
MicroProfileCreateThreadLogKey()
{
...
MICROPROFILE_DEFINE(g_MicroProfileFlip,
                    "MicroProfile",
                    "MicroProfileFlip",
                    MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileThreadLoop,
                    "MicroProfile",
                    "ThreadLoop",
                    MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileClear, "MicroProfile", "Clear", MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileAccumulate,
                    "MicroProfile",
                    "Accumulate",
                    MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileContextSwitchSearch,
                    ...
                    "ContextSwitchSearch",
                    MP_GREEN4);
MICROPROFILE_DEFINE(g_MicroProfileGpuSubmit,
                    "MicroProfile",
                    "MicroProfileGpuSubmit",
                    MP_HOTPINK2);
MICROPROFILE_DEFINE(g_MicroProfileSendLoop,
                    "MicroProfile",
                    "MicroProfileSocketSendLoop",
                    MP_GREEN4);
MICROPROFILE_DEFINE_LOCAL_ATOMIC_COUNTER(g_MicroProfileBytesPerFlip,
                                         "microprofile/bytesperflip");

void
MicroProfileHashTableInit(MicroProfileHashTable* pTable,
                          uint32_t nInitialSize,
                          MicroProfileHashCompareFunction CompareFunc,
                          MicroProfileHashFunction HashFunc);
void
MicroProfileHashTableDestroy(MicroProfileHashTable* pTable);
uint64_t
MicroProfileHashTableHash(MicroProfileHashTable* pTable, uint64_t K);
void
MicroProfileHashTableGrow(MicroProfileHashTable* pTable);

bool
MicroProfileHashTableSet(MicroProfileHashTable* pTable,
                         uint64_t Key,
                         uintptr_t Value,
                         uint64_t H,
                         bool bAllowGrow);
bool
MicroProfileHashTableGet(MicroProfileHashTable* pTable,
                         uint64_t Key,
                         uintptr_t* pValue);
bool
MicroProfileHashTableRemove(MicroProfileHashTable* pTable, uint64_t Key);

bool
MicroProfileHashTableSetString(MicroProfileHashTable* pTable,
                               const char* pKey,
                               const char* pValue);
bool
MicroProfileHashTableGetString(MicroProfileHashTable* pTable,
                               const char* pKey,
                               const char** pValue);
bool
MicroProfileHashTableRemoveString(MicroProfileHashTable* pTable,
                                  const char* pKey);
enum
{
...
const char*
MicroProfileStringIntern(const char* pStr);
const char*
MicroProfileStringInternLower(const char* pStr);
const char*
MicroProfileStringIntern(const char* pStr,
                         uint32_t nLen,
...
void
MicroProfileStringsInit(MicroProfileStrings* pStrings);
void
MicroProfileStringsDestroy(MicroProfileStrings* pStrings);

MicroProfileToken
MicroProfileCounterTokenInit(int nParent);
void
MicroProfileCounterTokenInitName(MicroProfileToken nToken, const char* pName);
void
MicroProfileCounterConfigInternal(MicroProfileToken,
                                  uint32_t eFormat,
                                  int64_t nLimit,
                                  uint32_t nFlags);
...
MicroProfileCreateThreadLog(const char* pName);
MicroProfileThreadLogGpu*
MicroProfileThreadLogGpuAllocInternal();
void*
MicroProfileSocketSenderThread(void*);
...
MicroProfileInit()
{
  ...
  if (bOnce) {
    ...
    for (int i = 0; i < MICROPROFILE_MAX_CATEGORIES; ++i) {
      ...
      memset(
        S.CategoryInfo[i].nGroupMask, 0, sizeof(S.CategoryInfo[i].nGroupMask));
    }
...
static void*
MicroProfileAutoFlipThread(void*)
{
...
void
MicroProfileStartAutoFlip(uint32_t nMsDelay)
{
...
#ifdef MICROPROFILE_IOS
inline MicroProfileThreadLog*
MicroProfileGetThreadLog()
{
...
inline void
MicroProfileSetThreadLog(MicroProfileThreadLog* pLog)
{
  pthread_once(&g_MicroProfileThreadLogKeyOnce, MicroProfileCreateThreadLogKey);
...
void
MicroProfileLogReset(MicroProfileThreadLog* pLog);
...
MicroProfileCreateThreadLog(const char* pName)
{
  ...
  if (pName) {
    ...
    len = snprintf(&pLog->ThreadName[0],
                   sizeof(pLog->ThreadName) - 1,
                   "TID:[%" PRId64 "]",
                   (int64_t)MP_GETCURRENTTHREADID());
  }
...
void
MicroProfileThreadLogGpuReset(MicroProfileThreadLogGpu* pLog)
{
...
void
MicroProfileThreadLogGpuFree(MicroProfileThreadLogGpu* pLog)
{
  MP_ASSERT(pLog->nAllocated);
  pLog->nAllocated = 0;
}
...
int
MicroProfileGetGpuQueue(const char* pQueueName)
{
  for (uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; i++) {
    MicroProfileThreadLog* pLog = S.Pool[i];
...
MicroProfileThreadLog*
MicroProfileGetGpuQueueLog(const char* pQueueName)
{
  for (uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; i++) {
    MicroProfileThreadLog* pLog = S.Pool[i];
    if (pLog && pLog->nGpu && pLog->nActive &&
...
int
MicroProfileInitGpuQueue(const char* pQueueName)
{
  for (uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i) {
    MicroProfileThreadLog* pLog = S.Pool[i];
...
void
MicroProfileFreeGpuQueue(int nQueue)
{
...
MicroProfileToken
MicroProfileFindToken(const char* pGroup, const char* pName)
{
  MicroProfileInit();
  MicroProfileScopeLock L(MicroProfileMutex());
...
uint16_t
MicroProfileGetGroup(const char* pGroup, MicroProfileTokenType Type)
{
...
void
MicroProfileRegisterGroup(const char* pGroup,
...
MicroProfileToken
MicroProfileGetToken(const char* pGroup,
                     const char* pName,
                     uint32_t nColor,
                     MicroProfileTokenType Type)
{
  ...
  snprintf(&S.TimerInfo[nTimerIndex].pNameExt[0],
           sizeof(S.TimerInfo[nTimerIndex].pNameExt) - 1,
           "%s %s",
...
void
MicroProfileGetTokenC(MicroProfileToken* pToken,
                      const char* pGroup,
                      const char* pName,
                      uint32_t nColor,
                      MicroProfileTokenType Type)
...
const char*
MicroProfileNextName(const char* pName, char* pNameOut, uint32_t* nSubNameLen)
{
...
const char*
MicroProfileCounterFullName(int nCounter)
{
...
MicroProfileToken
MicroProfileGetCounterTokenByParent(int nParent, const char* pName)
{
...
inline void
MicroProfileLogPut(MicroProfileLogEntry LE, MicroProfileThreadLog* pLog)
{
  MP_ASSERT(
    pLog !=
...
inline uint64_t
MicroProfileLogPutEnter(MicroProfileToken nToken_,
                        uint64_t nTick,
...
inline void
MicroProfileLogPutLeave(MicroProfileToken nToken_,
                        uint64_t nTick,
                        MicroProfileThreadLog* pLog)
{
  MP_ASSERT(
...
inline void
MicroProfileLogPut(MicroProfileToken nToken_,
                   uint64_t nTick,
                   uint64_t nBegin,
                   MicroProfileThreadLog* pLog)
{
...
inline void
MicroProfileLogPutGpu(MicroProfileLogEntry LE, MicroProfileThreadLogGpu* pLog)
{
...
inline void
MicroProfileLogPutGpu(MicroProfileToken nToken_,
                      uint64_t nTick,
                      uint64_t nBegin,
...
uint32_t
MicroProfileGroupTokenActive(MicroProfileToken nToken_)
{
...
uint64_t
MicroProfileEnterInternal(MicroProfileToken nToken_)
{
  if (MicroProfileGroupTokenActive(nToken_)) {
    ...
    if (MICROPROFILE_PLATFORM_MARKERS_ENABLED) {
      ...
      return MicroProfileLogPutEnter(
        nToken_, nTick, MicroProfileGetThreadLog2());
    }
  }
  return MICROPROFILE_INVALID_TICK;
}
...
void
MicroProfileTimelineLeave(uint32_t id)
{
  if (!id)
    return;
...
void
MicroProfileTimelineEnterStatic(uint32_t nColor, const char* pStr)
{
...
void
MicroProfileTimelineLeaveStatic(const char* pStr)
{
  for (uint32_t i = 0; i < MICROPROFILE_TIMELINE_MAX_TOKENS; ++i) {
...
uint32_t
MicroProfileTimelineEnterInternal(uint32_t nColor,
                                  const char* pStr,
                                  uint32_t nStrLen,
                                  int bIsStaticString)
{
...
uint32_t
MicroProfileTimelineEnter(uint32_t nColor, const char* pStr)
{
  return MicroProfileTimelineEnterInternal(
    nColor, pStr, (uint32_t)strlen(pStr), false);
}

uint32_t
MicroProfileTimelineEnterf(uint32_t nColor, const char* pStr, ...)
{
  ...
  buffer[size] = '\0';
  return MicroProfileTimelineEnterInternal(
    nColor, buffer, (uint32_t)size, false);
}

void
MicroProfileLocalCounterAdd(int64_t* pCounter, int64_t nCount)
{
  *pCounter += nCount;
}
int64_t
MicroProfileLocalCounterSet(int64_t* pCounter, int64_t nCount)
{
  int64_t r = *pCounter;
  *pCounter = nCount;
  return r;
...
void
MicroProfileLocalCounterAddAtomic(MicroProfileToken nToken, int64_t nCount)
{
  std::atomic<int64_t>* pCounter = &S.CounterInfo[nToken].ExternalAtomic;
  pCounter->fetch_add(nCount);
}
int64_t
MicroProfileLocalCounterSetAtomic(MicroProfileToken nToken, int64_t nCount)
{
...
void
MicroProfileCounterAdd(MicroProfileToken nToken, int64_t nCount)
{
  MP_ASSERT(nToken < S.nNumCounters);
  S.Counters[nToken].fetch_add(nCount);
}
void
MicroProfileCounterSet(MicroProfileToken nToken, int64_t nCount)
{
  MP_ASSERT(nToken < S.nNumCounters);
...
void
MicroProfileCounterSetLimit(MicroProfileToken nToken, int64_t nCount)
{
  MP_ASSERT(nToken < S.nNumCounters);
...
void
MicroProfileCounterConfigInternal(MicroProfileToken nToken,
                                  uint32_t eFormat,
                                  int64_t nLimit,
                                  uint32_t nFlags)
{
...
void
MicroProfileCounterConfig(const char* pName,
                          uint32_t eFormat,
                          int64_t nLimit,
                          uint32_t nFlags)
{
...
void
MicroProfileCounterSetPtr(const char* pCounterName,
                          void* pSource,
                          uint32_t nSize)
{
...
inline void
MicroProfileFetchCounter(uint32_t i)
{
...
void
MicroProfileLeaveInternal(MicroProfileToken nToken_, uint64_t nTickStart)
{
...
void
MicroProfileEnter(MicroProfileToken nToken)
{
  MicroProfileThreadLog* pLog = MicroProfileGetThreadLog2();
  MP_ASSERT(
    pLog->nStackScope <
...
void
MicroProfileEnterGpu(MicroProfileToken nToken, MicroProfileThreadLogGpu* pLog)
{
  MP_ASSERT(
    pLog->nStackScope <
    MICROPROFILE_STACK_MAX); // if youre hitting this assert you probably have
...
void
MicroProfileLeaveGpu(MicroProfileThreadLogGpu* pLog)
{
  MP_ASSERT(pLog->nStackScope > 0); // if youre hitting this assert you probably
                                    // have mismatched _ENTER/_LEAVE markers
  MicroProfileScopeStateC* pScopeState = &pLog->ScopeState[--pLog->nStackScope];
...
void
MicroProfileEnterNegative()
{
  MicroProfileEnter(S.nTokenNegativeCpu);
}

void
MicroProfileEnterNegativeGpu(MicroProfileThreadLogGpu* pLog)
{
  MicroProfileEnterGpu(S.nTokenNegativeGpu, pLog);
}

void
MicroProfileGpuBegin(void* pContext, MicroProfileThreadLogGpu* pLog)
{
...
void
MicroProfileGpuSetContext(void* pContext, MicroProfileThreadLogGpu* pLog)
{
  MP_ASSERT(pLog->pContext != (void*)-1); // dont call begin without calling end
  MP_ASSERT(pLog->nStart != (uint32_t)-1);
  pLog->pContext = pContext;
...
uint64_t
MicroProfileGpuEnd(MicroProfileThreadLogGpu* pLog)
{
...
void
MicroProfileGpuSubmit(int nQueue, uint64_t nWork)
{
...
uint64_t
MicroProfileGpuEnterInternal(MicroProfileThreadLogGpu* pGpuLog,
                             MicroProfileToken nToken_)
{
  if (MicroProfileGroupTokenActive(nToken_)) {
    ...
    MicroProfileLogPutGpu(
      ETOKEN_GPU_CPU_TIMESTAMP, MP_TICK(), MP_LOG_EXTRA_DATA, pGpuLog);
    MicroProfileLogPutGpu(ETOKEN_GPU_CPU_SOURCE_THREAD,
                          pLog->nLogIndex,
                          MP_LOG_EXTRA_DATA,
                          pGpuLog);
...
void
MicroProfileGpuLeaveInternal(MicroProfileThreadLogGpu* pGpuLog,
                             MicroProfileToken nToken_,
                             uint64_t nTickStart)
{
  if (nTickStart) {
...
void
MicroProfileContextSwitchPut(MicroProfileContextSwitch* pContextSwitch)
{
...
void
MicroProfileGetRange(uint32_t nPut, uint32_t nGet, uint32_t nRange[2][2])
{
  if (nPut > nGet) {
    nRange[0][0] = nGet;
    nRange[0][1] = nPut;
...
void
MicroProfileFlip(void* pContext)
{
...
void
MicroProfileFlip_CB(void* pContext, MicroProfileOnFreeze FreezeCB)
{
  ...
  if (nRunning) {
    ...
    pFramePut->nFrameStartGpu = nGpuTimeStamp;
    {
      ...
      float fTimeCpu =
        1000.f * (pFrameNext->nFrameStartCpu - pFrameCurrent->nFrameStartCpu) /
        MicroProfileTicksPerSecondCpu();
...
#define MP_ASSERT_LE_WRAP(l, g) MP_ASSERT(uint64_t(g - l) < 0x8000000000000000)
          ...
          for (uint32_t j = 0; j < 2; ++j) {
            ...
            for (uint32_t k = nStart; k < nEnd; ++k) {
              ...
              if (MicroProfileLogGetType(L) < MP_LOG_EXTRA_DATA) {
                pLog->Log[k] =
                  MicroProfileLogSetTick(L,
                                         MicroProfileGpuGetTimeStamp((
      ...
      }
      {
        ...
        for (uint32_t idx_thread = 0; idx_thread < MICROPROFILE_MAX_THREADS;
          ...
          if (nPut != nGet) {
            ...
            memset(
              pFrameGroupThread, 0, sizeof(S.FrameGroupThread[idx_thread]));
          }
  ...
  if (nAggregateFlip) {
    ...
    for (uint32_t i = 0; i < MICROPROFILE_MAX_THREADS; ++i) {
      ...
      memcpy(&pLog->nAggregateGroupTicks[0],
             &pLog->nGroupTicks[0],
    ...
    if (nAggregateClear) {
      ...
      memset(
        &S.AccumMaxTimers[0], 0, sizeof(S.AccumMaxTimers[0]) * S.nTotalTimers);
      memset(&S.AccumMinTimers[0],
             0xFF,
             sizeof(S.AccumMinTimers[0]) * S.nTotalTimers);
      memset(&S.AccumTimersExclusive[0],
             0,
             sizeof(S.AggregateExclusive[0]) * S.nTotalTimers);
      memset(&S.AccumMaxTimersExclusive[0],
             0,
...
void
MicroProfileSetEnableAllGroups(int bEnable)
{
  if (bEnable) {
    for (uint32_t i = 0; i < MICROPROFILE_MAX_GROUP_INTS; ++i) {
...
void
MicroProfileEnableCategory(const char* pCategory, int bEnabled)
{
  int nCategoryIndex = -1;
  for (uint32_t i = 0; i < S.nCategoryCount; ++i) {
    if (!MP_STRCASECMP(pCategory, S.CategoryInfo[i].pName)) {
...
void
MicroProfileEnableCategory(const char* pCategory)
{
  MicroProfileEnableCategory(pCategory, true);
}
void
MicroProfileDisableCategory(const char* pCategory)
{
...
void
MicroProfileSetForceMetaCounters(int bForce)
{}

int
MicroProfileGetForceMetaCounters()
{
...
void
MicroProfileEnableMetaCounter(const char* pMeta)
{}

void
MicroProfileDisableMetaCounter(const char* pMeta)
{}

void
MicroProfileSetAggregateFrames(int nFrames)
{
...
int
MicroProfileGetAggregateFrames()
{
  return S.nAggregateFlip;
}

int
MicroProfileGetCurrentAggregateFrames()
{
  return int(S.nAggregateFlip ? S.nAggregateFlip : S.nAggregateFlipCount);
}

void
MicroProfileForceEnableGroup(const char* pGroup, MicroProfileTokenType Type)
{
  MicroProfileInit();
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileMutex());
...
void
MicroProfileForceDisableGroup(const char* pGroup, MicroProfileTokenType Type)
{
  MicroProfileInit();
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileMutex());
  uint16_t nGroup = MicroProfileGetGroup(pGroup, Type);
...
void
MicroProfileCalcAllTimers(float* pTimers,
                          float* pAverage,
                          float* pMax,
                          float* pMin,
                          float* pCallAverage,
                          float* pExclusive,
                          float* pAverageExclusive,
                          float* pMaxExclusive,
                          float* pTotal,
                          uint32_t nSize)
{
...
float
MicroProfileGetTime(const char* pGroup, const char* pName)
{
...
void
MicroProfilePlatformMarkersSetEnabled(int bEnabled)
{
...
void
MicroProfileContextSwitchSearch(uint32_t* pContextSwitchStart,
...
int
MicroProfileFormatCounter(int eFormat,
                          int64_t nCounter,
                          char* pOut,
                          uint32_t nBufferSize)
{
...
typedef void
MicroProfileWriteCallback(void* Handle, size_t size, const char* pData);
...
void
MicroProfileDumpFileImmediately(const char* pHtml,
                                const char* pCsv,
                                void* pGpuContext)
{
...
void
MicroProfileDumpFile(const char* pHtml,
                     const char* pCsv,
                     float fCpuSpike,
                     float fGpuSpike)
{
...
void
MicroProfilePrintf(MicroProfileWriteCallback CB,
                   void* Handle,
                   const char* pFmt,
                   ...)
{
...
void
MicroProfileDumpCsv(MicroProfileWriteCallback CB, void* Handle)
{
  uint32_t nAggregateFrames = S.nAggregateFrames ? S.nAggregateFrames : 1;
  float fToMsCPU =
  ...
  MicroProfileCalcAllTimers(pTimers,
                            pAverage,
                            pMax,
                            pMin,
                            pCallAverage,
  ...
  for (uint32_t i = 0; i < S.nTotalTimers; ++i) {
    ...
    printf("\"%s\",\"%s\",%f,%f,%f\n",
           S.TimerInfo[i].pName,
           S.GroupInfo[S.TimerInfo[i].nGroupIndex].pName,
           pAverage[nIdx],
           pMax[nIdx],
           pCallAverage[nIdx]);
  }
...
void
MicroProfileDumpHtmlLive(MicroProfileWriteCallback CB, void* Handle)
{
  for (size_t i = 0; i < g_MicroProfileHtmlLive_begin_count; ++i) {
    CB(Handle,
       g_MicroProfileHtmlLive_begin_sizes[i] - 1,
       g_MicroProfileHtmlLive_begin[i]);
  }
  for (size_t i = 0; i < g_MicroProfileHtmlLive_end_count; ++i) {
    CB(Handle,
       g_MicroProfileHtmlLive_end_sizes[i] - 1,
       g_MicroProfileHtmlLive_end[i]);
  }
}
void
MicroProfileDumpHtml(MicroProfileWriteCallback CB,
                     void* Handle,
                     uint64_t nMaxFrames,
                     const char* pHost,
                     uint64_t nStartFrameId = (uint64_t)-1)
{
  ...
  for (size_t i = 0; i < g_MicroProfileHtml_begin_count; ++i) {
    CB(Handle,
       g_MicroProfileHtml_begin_sizes[i] - 1,
  ...
  // categories
  MicroProfilePrintf(
    CB, Handle, "S.CategoryInfo = Array(%d);\n", S.nCategoryCount);
  for (uint32_t i = 0; i < S.nCategoryCount; ++i) {
    MicroProfilePrintf(
      CB, Handle, "S.CategoryInfo[%d] = \"%s\";\n", i, S.CategoryInfo[i].pName);
  }
  ...
  for (uint32_t i = 0; i < S.nTotalTimers; ++i) {
    ...
    MicroProfilePrintf(CB,
                       ...
                       "'#%02x%02x%02x','#%02x%02x%02x', %f, %f, %f, %f, %f, "
                       "%f, %d, %f, S.Meta%d, S.MetaAvg%d, S.MetaMax%d);\n",
  ...
  for (uint32_t i = 0; i < S.nNumLogs; ++i) {
    MicroProfilePrintf(
      CB, Handle, "%d,", (S.Pool[i] && S.Pool[i]->nGpu) ? 1 : 0);
  }
  MicroProfilePrintf(CB, Handle, "];\n\n");
  ...
  for (uint32_t i = 0; i < S.nNumLogs; ++i) {
    if (S.Pool[i]) {
      ...
      for (uint32_t j = 0; j < MICROPROFILE_MAX_GROUPS; ++j) {
        MicroProfilePrintf(
          CB, Handle, "%f,", S.Pool[i]->nAggregateGroupTicks[j] * fToMs);
      }
      MicroProfilePrintf(CB, Handle, "];\n");
    }
  }
  ...
  for (int i = 0; i < (int)S.nNumCounters; ++i) {
    if (0 != (S.CounterInfo[i].nFlags & MICROPROFILE_COUNTER_FLAG_DETAILED)) {
      ...
      MicroProfilePrintf(
        CB,
        Handle,
        "S.CounterHistory%d = MakeCounterHistory(%d, S.CounterHistoryArray%d, "
        "S.CounterHistoryArrayPrc%d)\n",
        i,
        i,
        i,
        i);
  ...
  for (int i = 0; i < (int)S.nNumCounters; ++i) {
    ...
    MicroProfileFormatCounter(S.CounterInfo[i].eFormat,
                              S.CounterInfo[i].nLimit,
                              FormattedLimit,
                              sizeof(FormattedLimit) - 1);
    MicroProfilePrintf(
      CB,
      Handle,
...
#define pp(...) MicroProfilePrintf(CB, Handle, __VA_ARGS__)
  ...
  for (uint32_t i = 0; i < nNumFrames; ++i) {
    ...
    for (uint32_t j = 0; j < S.nNumLogs; ++j) {
      ...
      if (nLogStart != nLogEnd) {
        ...
        MicroProfilePrintf(
          CB, Handle, "%d", MicroProfileLogGetType(pLog->Log[k]));
    ...
    MicroProfilePrintf(
      CB,
      Handle,
      "S.Frames[%d] = MakeFrame(%d, %f, %f, %f, %f, S.ts%d, S.tt%d, S.ti%d);\n",
      i,
      0,
      fFrameMs,
      fFrameEndMs,
  ...
  for (uint32_t j = nContextSwitchStart; j != nContextSwitchEnd;
    ...
    int nCpu = CS.nCpu;
    MicroProfilePrintf(
      CB, Handle, "%d,%d,%d,", CS.nThreadIn, CS.nThreadOut, nCpu);
  }
  MicroProfilePrintf(CB, Handle, "];\n");
  ...
  std::sort(nGroupCounterSort,
            nGroupCounterSort + S.nGroupCount,
            [nGroupCounter](const uint32_t l, const uint32_t r) {
              return nGroupCounter[l] > nGroupCounter[r];
            });

  std::sort(nTimerCounterSort,
            nTimerCounterSort + S.nTotalTimers,
...
void
MicroProfileWriteFile(void* Handle, size_t nSize, const char* pData)
{
...
MicroProfileDumpToFile()
{
  ...
  if (S.nDumpFileNextFrame & 1) {
    ...
    if (F) {
      MicroProfileDumpHtml(MicroProfileWriteFile,
                           F,
                           MICROPROFILE_WEBSERVER_MAXFRAMES,
                           S.HtmlDumpPath);
      fclose(F);
    }
  }
...
void
MicroProfileFlushSocket(MpSocket Socket)
{
...
void
MicroProfileWriteSocket(void* Handle, size_t nSize, const char* pData)
{
...
void
MicroProfileCompressedSocketFlush(MicroProfileCompressedSocketState* pState)
{
...
void
MicroProfileCompressedSocketStart(MicroProfileCompressedSocketState* pState,
                                  MpSocket Socket)
{
  mz_stream& Stream = pState->Stream;
...
void
MicroProfileCompressedSocketFinish(MicroProfileCompressedSocketState* pState)
{
  mz_stream& Stream = pState->Stream;
...
void
MicroProfileCompressedWriteSocket(void* Handle, size_t nSize, const char* pData)
{
  ...
  while (nSize) {
    ...
    if (r == MZ_BUF_ERROR) {
      r = mz_deflate(&Stream, MZ_SYNC_FLUSH);
                                   ...
                                   // platforms..
void
MicroProfileSetNonBlocking(MpSocket Socket, int NonBlocking)
{
#ifdef _WIN32
...
#if defined(_WIN32)
    r = setsockopt(
      S.ListenerSocket, SOL_SOCKET, SO_REUSEADDR, (const char*)&on, sizeof(on));
#else
    r = setsockopt(
      S.ListenerSocket, SOL_SOCKET, SO_REUSEADDR, (void*)&on, sizeof(on));
#endif
    (void)r;
  }
...
void
MicroProfileBase64Encode(char* pOut, const uint8_t* pIn, uint32_t nLen)
{
...
static void
MicroProfile_SHA1_Transform(uint32_t state[5], const unsigned char buffer[64])
{
  uint32_t a, b, c, d, e;
  typedef union
  {
...
void
MicroProfile_SHA1_Init(MicroProfile_SHA1_CTX* context)
{
...
void
MicroProfile_SHA1_Update(MicroProfile_SHA1_CTX* context,
...
void
MicroProfile_SHA1_Final(unsigned char digest[20],
...
void
MicroProfileWebSocketSendState(MpSocket C);
void
MicroProfileWebSocketSendEnabled(MpSocket C);
void
MicroProfileWSPrintStart(MpSocket C);
void
MicroProfileWSPrintf(const char* pFmt, ...);
void
MicroProfileWSPrintEnd();
void
MicroProfileWSFlush();
bool
MicroProfileWebSocketReceive(MpSocket C);
...
bool
MicroProfileSocketSend2(MpSocket Connection, const void* pMessage, int nLen);
void*
MicroProfileSocketSenderThread(void*)
{
...
void
MicroProfileSocketSend(MpSocket Connection, const void* pMessage, int nLen)
{
...
uint32_t
MicroProfileWebSocketIdPack(uint32_t type, uint32_t element)
{
  MP_ASSERT(type < 255);
...
void
MicroProfileWebSocketIdUnpack(uint32_t nPacked,
                              uint32_t& type,
                              uint32_t& element)
{
...
bool
MicroProfileWebSocketSend(MpSocket Connection,
                          const char* pMessage,
                          uint64_t nLen)
{
...
void
MicroProfileToggleWebSocketToggleTimer(uint32_t nTimer)
{
...
inline bool
MicroProfileGroupActive(uint32_t nGroupIndex)
{
...
bool
MicroProfileWebSocketTimerEnabled(uint32_t nTimer)
{
  if (nTimer < S.nTotalTimers) {
...
void
MicroProfileToggleGroup(uint32_t nGroup)
{
...
bool
MicroProfileGroupEnabled(uint32_t nGroup)
{
  if (nGroup < S.nGroupCount) {
    uint32_t nIndex = nGroup / 32;
    uint32_t nBit = nGroup % 32;
...
bool
MicroProfileCategoryEnabled(uint32_t nCategory)
{
...
void
MicroProfileToggleCategory(uint32_t nCategory)
{
  if (nCategory < S.nCategoryCount) {
    bool bAllSet = true;
    for (uint32_t i = 0; i < MICROPROFILE_MAX_GROUP_INTS; ++i) {
...
void
MicroProfileWebSocketCommand(uint32_t nCommand)
{
...
typedef bool (*MicroProfileOnSettings)(const char* pName,
                                       uint32_t nNameLen,
                                       const char* pJson,
                                       uint32_t nJson);
...
template<typename T>
void
MicroProfileParseSettings(const char* pFileName, T CB)
{
  ...
  while (pPos != pEnd) {
    ...
    auto ParseName =
      [&](char* pPos, char* pEnd, const char** ppName, int* pLen) {
    ...
    auto ParseJson =
      [&](char* pPos, char* pEnd, const char** pJson, int* pLen) -> char* {
      pPos = SkipWhite(pPos, pEnd);
      if (*pPos != '{' || pPos == pEnd) {
        Failed = 1;
        return pPos;
...
bool
MicroProfileSavePresets(const char* pSettingsName, const char* pJsonSettings)
{
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileGetMutex());
  ...
  MicroProfileParseSettings(MICROPROFILE_SETTINGS_FILE,
                            [&](const char* pName,
                              ...
                              if (0 != MP_STRCASECMP(pSettingsName, pName)) {
                                ...
                                bWritten = true;
                                fwrite(
                                  pJsonSettings, strlen(pJsonSettings), 1, F);
                              }
                              fputc('\n', F);
                              return true;
                            });
...
#elif defined(_WIN32)
  MoveFileExA(MICROPROFILE_SETTINGS_FILE_TEMP,
              MICROPROFILE_SETTINGS_FILE,
...
void
MicroProfileWriteJsonString(const char* pJson, uint32_t nJsonLen)
{
...
void
MicroProfileWebSocketSendPresets(MpSocket Connection)
{
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileGetMutex());
...
void
MicroProfileLoadPresets(const char* pSettingsName, bool bReadOnlyPreset)
{
  std::lock_guard<std::recursive_mutex> Lock(MicroProfileGetMutex());
...
bool
MicroProfileWebSocketReceive(MpSocket Connection)
{
  ...
  switch (Bytes[0]) {
    ...
    case 'D': // instrumentation without loading queryable symbols.
    {
      ...
      MicroProfileInstrumentWithoutSymbols(
        pModules, pSymbols, nParsedArguments);
...
void
MicroProfileWebSocketHandshake(MpSocket Connection, char* pWebSocketKey)
{
  ...
  int nLen = snprintf(
    EncodeBuffer, sizeof(EncodeBuffer) - 1, "%s%s", pWebSocketKey, pGUID);
  ...
  char Reply[11024];
  nLen =
    snprintf(Reply, sizeof(Reply) - 1, "%s%s\r\n\r\n", pHandShake, HashOut);
  ;
  MP_ASSERT(nLen >= 0);
...
{
  MICROPROFILE_SCOPEI(
    "MicroProfile", "MicroProfileWebSocketSendCounters", MP_GREEN4);
...
#define FMT "{\"n\":\"%s\",\"a\":\"%llx\",\"s\":\"%lld\", \"p\":%f}"
      MicroProfileWSPrintf(bFirst ? FMT : ("," FMT),
                           pModuleName,
                           nAddrBegin,
                           nNumSymbols,
                           fLoadPrc);
...
void
MicroProfileWebSocketSendFrame(MpSocket Connection)
{
  if (S.nFrameCurrent != S.WebSocketFrameLast[0] || S.nFrozen) {
    MicroProfileWebSocketSendState(Connection);
    MICROPROFILE_SCOPEI(
      "MicroProfile", "MicroProfileWebSocketSendFrame", MP_GREEN4);
    MicroProfileWSPrintStart(Connection);
    float fTickToMsCpu =
      MicroProfileTickToMsMultiplier(MicroProfileTicksPerSecondCpu());
    float fTickToMsGpu =
    ...
    if (S.nFrameCurrent != S.WebSocketFrameLast[0]) {
      ...
      while (-1 != nTimer) {
        ...
        nTimer = TI.nWSNext;
        MicroProfileWSPrintf("\"%d\":[%f,%f,%f]%c",
                             id,
                             fTime,
                             fTimeExcl,
                             fCount,
...
MicroProfileWSPrintf(const char* pFmt, ...)
{
  ...
  if (S.WSBuf.nPut + nRequiredSize + 2 > S.WSBuf.nBufferSize) {
    uint32_t nNewSize = MicroProfileMax(
      S.WSBuf.nPut + 2 * (nRequiredSize + 2 + 20),
...
void
MicroProfileWebSocketSendEnabledMessage(uint32_t id, int bEnabled)
{
  MicroProfileWSPrintf("{\"k\":\"%d\",\"v\":{\"id\":%d,\"e\":%d}}",
                       MSG_ENABLED,
                       id,
                       bEnabled ? 1 : 0);
  MicroProfileWSFlush();
...
void
MicroProfileWebSocketSendEntry(uint32_t id,
                               uint32_t parent,
                               const char* pName,
                               int nEnabled,
                               uint32_t nColor)
{
  MicroProfileWSPrintf(
    "{\"k\":\"%d\",\"v\":{\"id\":%d,\"pid\":%d,", MSG_TIMER_TREE, id, parent);
...
void
MicroProfileWebSocketSendCounterEntry(uint32_t id,
                                      uint32_t parent,
                                      const char* pName,
                                      int64_t nLimit,
                                      int nFormat)
  ...
  if (S.WSCategoriesSent != S.nCategoryCount ||
    ...
    for (uint32_t i = S.WSCountersSent; i < S.nNumCounters; ++i) {
      ...
      MicroProfileWebSocketSendCounterEntry(
        id, parent, CI.pName, CI.nLimit, CI.eFormat);
    }
        ...
        *pHttp = '\0';
        ...
        switch (P) {
          case EMICROPROFILE_GET_COMMAND_LIVE: {
            ...
            send(Connection,
                 MICROPROFILE_HTML_HEADER,
                 sizeof(MICROPROFILE_HTML_HEADER) - 1,
                 0);
            ...
            MicroProfilePrintf(
              MicroProfileCompressedWriteSocket,
              &CompressState,
...
void*
MicroProfileTraceThread(void* unused);
int
MicroProfileIsLocalThread(uint32_t nThreadId);
...
static GUID g_MicroProfileThreadClassGuid = { 0x3d6fa8d1, 0xfe05, 0x11d0, 0x9d,
                                              0xda,       0x00,   0xc0,   0x4f,
                                              0xd7,       0xba,   0x7c };
...
VOID WINAPI
MicroProfileContextSwitchCallback(PEVENT_TRACE pEvent)
{
...
ULONG WINAPI
MicroProfileBufferCallback(PEVENT_TRACE_LOGFILE Buffer)
{
...
bool
MicroProfileStartWin32Trace(EventCallback EvtCb, BufferCallback BufferCB)
{
  MicroProfileContextSwitchShutdownTrace();
  ...
  status = StartTrace(
    (PTRACEHANDLE)&SessionHandle, KERNEL_LOGGER_NAME, &sessionProperties);
...
const char*
MicroProfileWin32ThreadInfoAddString(const char* pString)
{
  ...
  for (uint32_t i = 0; i < MAX_SEARCH; ++i) {
    ...
    if (0 == g_ThreadInfo.pStrings[idx]) {
      ...
      memcpy(&g_ThreadInfo.StringData[g_ThreadInfo.nStringOffset],
             pString,
             nLen + 1);
...
void
MicroProfileWin32ExtractModules(MicroProfileWin32ThreadInfo::Process& P)
{
...
const char*
MicroProfileThreadNameFromId(MicroProfileThreadIdType nThreadId)
{
  MicroProfileWin32UpdateThreadInfo();
  ...
  for (uint32_t i = 0; i < g_ThreadInfo.nNumThreads; ++i) {
    if (g_ThreadInfo.T[i].tid == nThreadId) {
      sprintf_s(result,
                "p:%s t:%s",
                g_ThreadInfo.T[i].pProcessModule,
...
VOID WINAPI
MicroProfileContextSwitchCallbackCollector(PEVENT_TRACE pEvent)
{
    ...
    if (pEvent->Header.Class.Type == 36) {
      ...
      if ((pCSwitch->NewThreadId != 0) || (pCSwitch->OldThreadId != 0)) {
        ...
        if (nPut - nGet < MicroProfileWin32ContextSwitchShared::BUFFER_SIZE) {
          g_pShared
            ->Buffer[nPut % MicroProfileWin32ContextSwitchShared::BUFFER_SIZE] =
            Switch;
...
ULONG WINAPI
MicroProfileBufferCallbackCollector(PEVENT_TRACE_LOGFILE Buffer)
{
...
MicroProfileTraceThread(void* unused)
{
  ...
  if (!MicroProfileStartWin32Trace(MicroProfileContextSwitchCallback,
    ...
    time_t t = time(NULL);
    _snprintf_s(
      Filename, sizeof(Filename), "%s_%d", MICROPROFILE_FILEMAPPING, (int)t);
...
#define CSWITCH_EXE "microprofile-win32-cswitch_x64.exe"
        ...
        HINSTANCE Instance = ShellExecuteA(
          NULL, "runas", CSWITCH_EXE, Filename, "", SW_SHOWMINNOACTIVE);
...
uint32_t
MicroProfileGpuInsertTimeStamp(void* pContext_)
{
  if (S.pGPU) {
...
uint64_t
MicroProfileGpuGetTimeStamp(uint32_t nIndex)
{
...
bool
MicroProfileGpuGetData(void* pQuery, void* pData, uint32_t nDataSize)
{
  HRESULT hr;
  do {
...
uint32_t
MicroProfileGpuFlip(void* pDeviceContext_)
{
  if (!pDeviceContext_) {
    return (uint32_t)-1;
  }
  ...
  if (OldFrame.m_nRateQueryStarted) {
    ...
    if (MicroProfileGpuGetData(
          OldFrame.m_pRateQuery, &Result, sizeof(Result))) {
...
void
MicroProfileGpuInitD3D11(void* pDevice_, void* pImmediateContext)
{
...
int
MicroProfileGetGpuTickReference(int64_t* pOutCPU, int64_t* pOutGpu)
{
  if (S.pGPU) {
    MicroProfileD3D11Frame& Frame =
      S.pGPU->m_QueryFrames[S.pGPU->m_nQueryFrame];
    if (Frame.m_nRateQueryStarted) {
      ...
      do {
        hr =
          pImmediateContext->GetData(pSyncQuery, pOutGpu, sizeof(*pOutGpu), 0);
      } while (hr == S_FALSE);
      *pOutCPU = MP_TICK();
      switch (hr) {
        case DXGI_ERROR_DEVICE_REMOVED:
...
#include <d3d12.h>
uint32_t
MicroProfileGpuInsertTimeStamp(void* pContext)
{
...
void
MicroProfileGpuFetchRange(uint32_t nBegin,
                          int32_t nCount,
                          uint64_t nFrame,
                          int64_t nTimestampOffset)
{
  ...
  memcpy(&S.pGPU->nResults[nBegin],
         nBegin + (uint64_t*)pData,
...
void
MicroProfileGpuWaitFence(uint32_t nNode, uint64_t nFence)
{
...
void
MicroProfileGpuFetchResults(uint64_t nFrame)
{
...
uint32_t
MicroProfileGpuFlip(void* pContext)
{
  uint32_t nNode = S.pGPU->nCurrentNode;
...
void
MicroProfileGpuInitD3D12(void* pDevice_,
                         uint32_t nNodeCount,
                         void** pCommandQueues_)
{
  ...
  for (uint32_t nNode = 0; nNode < S.pGPU->nNodeCount; ++nNode) {
    ...
    pDevice->CreateFence(S.pGPU->nPendingFrame,
...
void
MicroProfileSetCurrentNodeD3D12(uint32_t nNode)
{
...
void
MicroProfileGpuFetchRange(VkCommandBuffer CommandBuffer,
                          uint32_t nNode,
                          uint32_t nBegin,
                          int32_t nCount,
                          uint64_t nFrame,
                          int64_t nTimestampOffset)
{
  ...
  vkGetQueryPoolResults(S.pGPU->Devices[nNode],
                        S.pGPU->QueryPool[nNode],
                        nBegin,
                        nCount,
                        8 * nCount,
                        &S.pGPU->nResults[nBegin],
                        8,
...
void
MicroProfileGpuWaitFence(uint32_t nNode, uint64_t nFrame)
{
  int r;
  int c = 0;
  do {
    MICROPROFILE_SCOPEI("Microprofile", "gpu-wait", MP_GREEN4);
    r = vkWaitForFences(S.pGPU->Devices[nNode],
                        1,
                        &S.pGPU->Frames[nFrame].Fences[nNode],
                        1,
                        1000 * 30);
#if 0
		if(c++ > 1000 && (c%100) == 0)
...
void
MicroProfileGpuFetchResults(VkCommandBuffer Buffer, uint64_t nFrame)
{
  uint64_t nPending = S.pGPU->nPendingFrame;
  // while(nPending <= nFrame)
  // while(0 <= nFrame - nPending)
  while (0 <= (int64_t)(nFrame - nPending)) {
    ...
    uint32_t nBegin = S.pGPU->Frames[nInternal].nBegin;
    uint32_t nCount = S.pGPU->Frames[nInternal].nCount;
    MicroProfileGpuFetchRange(Buffer,
                              nNode,
                              nBegin,
                              (nBegin + nCount) >
                                  MICROPROFILE_VULKAN_MAX_QUERIES
...
void
MicroProfileGpuInitVulkan(VkDevice* pDevices,
                          ...
                          uint32_t nNodeCount)
{
  ...
  for (uint32_t i = 0; i < nNodeCount; ++i) {
    ...
    r = vkCreateCommandPool(
      S.pGPU->Devices[i], &CreateInfo, 0, &S.pGPU->CommandPool[i]);
    MP_ASSERT(r == VK_SUCCESS);
    ...
    for (uint32_t j = 0; j < MICROPROFILE_VULKAN_INTERNAL_DELAY; ++j) {
      ...
      r = vkAllocateCommandBuffers(
        S.pGPU->Devices[i], &AllocInfo, &F.CommandBuffer[i]);
      MP_ASSERT(r == VK_SUCCESS);
      ...
      if (j == 0) {
        ...
        vkCmdResetQueryPool(F.CommandBuffer[i],
                            S.pGPU->QueryPool[i],
                            0,
...
void
MicroProfileSetCurrentNodeVulkan(uint32_t nNode)
{
  S.pGPU->nCurrentNode = nNode;
}
...
MicroProfileGetGpuTickReference(int64_t* pOutCPU, int64_t* pOutGpu)
{
  ...
  vkCmdResetQueryPool(
    CB, S.pGPU->QueryPool[nGpu], MICROPROFILE_VULKAN_MAX_QUERIES, 1);
  vkCmdWriteTimestamp(CB,
                      VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
                      S.pGPU->QueryPool[nGpu],
                      MICROPROFILE_VULKAN_MAX_QUERIES);
...
uint64_t
MicroProfileGpuGetTimeStamp(uint32_t nKey)
{
  uint64_t result;
...
const char*
MicroProfileStrDup(const char* pStr)
{
...
void*
MicroProfileX64FollowJump(void* pSrc);
bool
MicroProfileCopyInstructionBytes(char* pDest,
                                 void* pSrc,
                                 const int nLimit,
                                 const int nMaxSize,
                                 char* pTrunk,
                                 ...
                                 uint32_t* nRetSafe);
bool
MicroProfilePatchFunction(void* f,
                          int Argument,
...
void
MicroProfileIterateSymbols(Callback CB,
                           uint32_t* nModules,
                           uint32_t nNumModules);
const char*
MicroProfileGetUnmangledSymbolName(void* pFunction);
...
extern "C" MP_NOINLINE uintptr_t
MicroProfile_Patch_TLS_PUSH(uintptr_t t)
{
...
char*
MicroProfileInsertRegisterJump(char* pCode, intptr_t pDest, int reg)
{
...
char*
MicroProfileInsertRelativeJump(char* pCode, intptr_t pDest)
{
...
char*
MicroProfileInsertRetJump(char* pCode, intptr_t pDest)
{
...
uint8_t*
MicroProfileInsertMov(uint8_t* p, uint8_t* pend, int r, intptr_t value)
{
...
uint8_t*
MicroProfileInsertCall(uint8_t* p, uint8_t* pend, int r)
{
  int Large = r >= R_R8 ? 1 : 0;
  int RegIndex = Large ? (r - R_R8) : (r - R_RAX);
  if (Large) {
...
bool
MicroProfileStringMatch(const char* pSymbol,
                        uint32_t nStartOffset,
                        const char** pPatterns,
...
int
MicroProfileStringMatchOffset(const char* pSymbol,
                              const char** pPatterns,
                              uint32_t* nPatternLength,
                              uint32_t nNumPatterns)
{
    ...
    switch (I.ops[0].type) {
      case O_REG: {
        ...
        switch (O2.type) {
          ...
          case O_MEM:
          case O_SMEM: {
            // if register is RSP 'contaminated', it prevents us from using that
            // to do retjmps
        ...
        case O_MEM: {
          if (O.index == R_RIP) {
            ...
            if (I.dispSize != 0x20 && I.dispSize != 0x10) {
              uprintf(
                "found offset size != 32 && != 16 bit. not implemented\n");
              BREAK_ON_PATCH_FAIL();
              return false;
            }
            rip[i] = 1;
...
extern "C" void
MicroProfileInterceptEnter(int a)
{
...
extern "C" void
MicroProfileInterceptLeave(int a)
{
  MicroProfileThreadLog* pLog = MicroProfileGetThreadLog2();
  MP_ASSERT(pLog->nStackScope > 0); // if youre hitting this assert you probably
                                    // have mismatched _ENTER/_LEAVE markers
...
void
MicroProfileInstrumentFromAddressOnly(void* pFunction)
{
  ...
  if (pDesc) {
    ...
    MicroProfileInstrumentFunction(
      pFunction,
      MicroProfileSymbolModuleGetString(pDesc->nModule),
      pDesc->pName,
      nColor);
  } else {
                               ...
                               uint32_t nColor)
{
  ...
  if (MicroProfilePatchFunction(pFunction,
                                S.DynamicTokenIndex,
                                MicroProfileInterceptEnter,
                                MicroProfileInterceptLeave,
                                &Err)) {
    MicroProfileToken Tok = S.DynamicTokens[S.DynamicTokenIndex] =
      MicroProfileGetToken(
        "PATCHED", pFunctionName, nColor, MicroProfileTokenTypeCpu);
...
bool
MicroProfileSymbolInitialize(bool bStartLoad, const char* pModuleName)
{
  if (!bStartLoad)
  ...
  // int nRequests = 0;
  {
    ...
    for (int i = 0; i < S.SymbolNumModules; ++i) {
      if (0 == pModuleName ||
          0 ==
            strcmp(pModuleName, (const char*)S.SymbolModules[i].pBaseString)) {
...
int
MicroProfileCharacterMaskCharIndex(char c)
{
  if (c >= 'A' && c <= 'Z')
    c = 'a' + (c - 'A');
...
uint64_t
MicroProfileCharacterMaskChar(char c)
{
  uint64_t nMask = 1;
...
uint_string_match
MicroProfileCharacterMaskString(const char* pStr)
{
...
void
MicroProfileCharacterMaskString2(const char* pStr,
...
bool
MicroProfileCharacterMatch(const MicroProfileStringMatchMask& Block,
...
uint32_t
MicroProfileSymbolGetModule(const char* pString, intptr_t nBaseAddr)
{
...
MicroProfileSymbolMergeExecutableRegions()
{
  for (int i = 0; i < S.SymbolNumModules; ++i) {
    ...
    if (M.nNumExecutableRegions > 1) {
      std::sort(&M.Regions[0],
                &M.Regions[M.nNumExecutableRegions],
...
uint32_t
MicroProfileSymbolInitModule(const char* pString_,
                             intptr_t nAddrBegin,
                             intptr_t nAddrEnd)
{
...
MicroProfileSymbolInitializeInternal()
{
  ...
  auto AllocBlock = []() -> MicroProfileSymbolBlock* {
    ...
    MICROPROFILE_COUNTER_ADD("/MicroProfile/Symbols/Memory",
                             sizeof(MicroProfileSymbolBlock));
    MICROPROFILE_COUNTER_CONFIG_ONCE(
      "/MicroProfile/Symbols/Memory", MICROPROFILE_COUNTER_FORMAT_BYTES, 0, 0);
    memset(pBlock, 0, sizeof(MicroProfileSymbolBlock));
    return pBlock;
  };

  auto SymbolCallback = [&](const char* pName,
                            const char* pShortName,
                            intptr_t nAddress,
                            intptr_t nAddressEnd,
                            uint32_t nModuleId) {
    ...
    char* pStr =
      &pActiveBlock
         ->Chars[MicroProfileSymbolBlock::ESIZE - pActiveBlock->nNumChars - 1];
...
#undef SYMDBG
  ...
  do {
    ...
    MicroProfileIterateSymbols(
      SymbolCallback, nModuleLoad, nNumModulesRequested);
...
struct MicroProfileFunctionQuery
{
  ...
  uint32_t nModuleFilterMatch
    [MICROPROFILE_INSTRUMENT_MAX_MODULES]; // prematch the modules, so it can be
...
MicroProfileAllocFunctionQuery()
{
  MicroProfileScopeLock L(MicroProfileMutex());
...
void
MicroProfileFreeFunctionQuery(MicroProfileFunctionQuery* pQ)
{
...
void
MicroProfileProcessQuery(MicroProfileFunctionQuery* pQuery)
{
          ...
          for (uint32_t i = 0;
            ...
            if (0 == E.nIgnoreSymbol) {
              ...
              if (nMaskQ == (nMaskQ & E.nMask)) {
                ...
                if (MicroProfileStringMatch(E.pShortName,
                                            nModuleMatchOffset,
                                            &Q.pFilterStrings[0],
                                            Q.nPatternLength,
                                            Q.nMaxFilter)) {
...
void*
MicroProfileQueryThread(void* p)
{
...
MicroProfileSymbolSendErrors(MpSocket Connection)
{
  if (S.nNumPatchErrors) {
    ...
    MicroProfileWSPrintf(
      ...
      MSG_INSTRUMENT_ERROR,
      MICROPROFILE_MAJOR_VERSION,
...
{
  MICROPROFILE_SCOPEI(
    "MicroProfile", "MicroProfileSymbolQuerySendResult", MP_PINK2);
  ...
  if (pQuery) {
    ...
    MicroProfileWSPrintStart(Connection);
    MicroProfileWSPrintf(
      "{\"k\":\"%d\",\"q\":%d,\"v\":[", MSG_FUNCTION_RESULTS, pQuery->QueryId);
    bool bFirst = true;
...
{
  MICROPROFILE_SCOPEI(
    "MicroProfile", "MicroProfileSymbolQueryFunctions", MP_WHEAT);
  ...
  }
  {
    ...
    if (0 == S.pPendingQuery || S.pPendingQuery->QueryId < (uint32_t)QueryId) {
      ...
      for (int i = 0; i < S.SymbolNumModules; ++i) {
        Q.nModuleFilterMatch[i] =
          MicroProfileStringMatchOffset(MicroProfileSymbolModuleGetString(i),
                                        Q.pFilterStrings,
                                        Q.nPatternLength,
                                        Q.nMaxFilter);
      }
...
#ifdef _WIN32
static void*
MicroProfileAllocExecutableMemory(void* pBase, size_t s);
static void*
MicroProfileAllocExecutableMemoryFar(size_t s);
static void
MicroProfileMakeMemoryExecutable(void* p, size_t s);
static void
MicroProfileMakeWriteable(void* p_, size_t size, DWORD* oldFlags);
static void
MicroProfileRestore(void* p_, size_t size, DWORD* oldFlags);
                          ...
                          MicroProfilePatchError* pError)
{
  ...
  if (!MicroProfileCopyInstructionBytes(pInstructionMoveDest,
                                        f,
                                        nBytesToCopy,
                                        (int)codemaxsize,
                                        pTrunk,
                                        t_trunk_size,
                                        nUsableJumpRegs,
                                        &nInstructionBytesDest,
                                        &nInstructionBytesSrc,
                                        &nRegsWritten,
                                        &nRetSafe)) {
    if (pError) {
      ...
      int off = snprintf(pError->Message,
                         sizeof(pError->Message),
                         "Failed to move %d code bytes ",
                         nInstructionBytesSrc);
      pError->nCodeSize = nInstructionBytesSrc;
      for (int i = 0; i < nInstructionBytesSrc; ++i) {
        off += snprintf(off + pError->Message,
                        sizeof(pError->Message) - off,
                        "%02x ",
                        0xff & pCode[i]);
      }
      uprintf("%s\n", pError->Message);
  ...
  if (0 == reg) {
    ...
    MicroProfileInsertRegisterJump(
      pInstructionMoveDest + nInstructionBytesDest, phome, r);
  }
  ...
  memcpy(
    (char*)ptramp + t_code_call_patch_push_offset + 2, &patch_push_addr, 8);
...
static void
MicroProfileMakeWriteable(void* p_, size_t s, DWORD* oldFlags)
{
  ...
  for (uint32_t i = 0; i < nNumPages; ++i) {
    if (!VirtualProtect((void*)(aligned + nPageSize * i),
                        nPageSize,
...
static void
MicroProfileRestore(void* p_, size_t s, DWORD* oldFlags)
{
  static uint64_t nPageSize = 4 << 10;
  ...
  for (uint32_t i = 0; i < nNumPages; ++i) {
    if (!VirtualProtect(
          (void*)(aligned + nPageSize * i), nPageSize, oldFlags[i], &Dummy)) {
      MP_BREAK();
    }
  }
}
...
void*
MicroProfileAllocExecutableMemoryUp(intptr_t nBase, size_t s)
{
  ...
  while (nBase < nEnd) {
    ...
    if (mbi.State == MEM_FREE && mbi.RegionSize >= s) {
      void* pMemory =
        VirtualAlloc((void*)nBase, s, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
      if (pMemory) {
        return pMemory;
      }
    }
...
void*
MicroProfileAllocExecutableMemoryDown(intptr_t nBase, size_t s)
{
  intptr_t nEnd = nBase - 0x80000000;
  MEMORY_BASIC_INFORMATION mbi;
  while (nBase > nEnd) {
...
int
MicroProfileTrimFunctionName(const char* pStr, char* pOutBegin, char* pOutEnd)
{
...
int
MicroProfileFindFunctionName(const char* pStr, const char** ppStart)
{
  int l = (int)strlen(pStr) - 1;
  if (l < 1024) {
    char b[1024] = { 0 };
...
BOOL CALLBACK
MicroProfileEnumModules(_In_ PCTSTR ModuleName,
                        ...
                        _In_opt_ PVOID UserContext)
{
  ...
  GetModuleInformation(
    GetCurrentProcess(), (HMODULE)BaseOfDll, &MI, sizeof(MI));
  MEMORY_BASIC_INFORMATION B;
  int r =
    VirtualQuery((LPCVOID)BaseOfDll, (MEMORY_BASIC_INFORMATION*)&B, sizeof(B));
  char buffer[1024];
  int r1 = GetLastError();
  if (r == 0) {
  ...
  MicroProfileSymbolInitModule(
    ModuleName, BaseOfDll, BaseOfDll + MI.SizeOfImage);
  return true;
}
...
{
  virtual void CB(const char* pName,
                  const char* pShortName,
                  intptr_t addr,
                  intptr_t addrend,
...
BOOL
MicroProfileQueryContextEnumSymbols(_In_ PSYMBOL_INFO pSymInfo,
                                    _In_ ULONG SymbolSize,
                                    _In_opt_ PVOID UserContext)
{
  uint32_t nModuleId = nLastModuleIdWin32;
  if (nLastModuleBaseWin32 != (intptr_t)pSymInfo->ModBase) {
    nLastModuleIdWin32 = nModuleId =
      MicroProfileSymbolGetModule((const char*)(intptr_t)-2, pSymInfo->ModBase);
  ...
  if (pSymInfo->Tag == 5 || pSymInfo->Tag == 10) {
    ...
    int ret = 0;
    int l = MicroProfileTrimFunctionName(
      pSymInfo->Name, &FunctionName[0], &FunctionName[1024]);
    QueryCallbackBase* pCB = (QueryCallbackBase*)UserContext;

    pCB->CB(pSymInfo->Name,
            l ? &FunctionName[0] : 0,
            (intptr_t)pSymInfo->Address,
            pSymInfo->Size + (intptr_t)pSymInfo->Address,
            nModuleId);
  }
  return TRUE;
};
                           ...
                           uint32_t nNumModules)
{
  ...
  if (MicroProfileSymInit()) {
    ...
    if (SymEnumerateModules64(
          GetCurrentProcess(),
          (PSYM_ENUMMODULES_CALLBACK64)MicroProfileEnumModules,
          NULL)) {
    }
          ...
          while (b < e) {
            ...
            switch (B.Protect) {
              ...
              case PAGE_EXECUTE_WRITECOPY:
                ...
                // B.BaseAddress, (void*)(intptr_t(B.BaseAddress) +
                // B.RegionSize), B.RegionSize / 1024.f, B.State, B.Protect);
            }
...
BOOL
MicroProfileQueryContextEnumSymbols1(_In_ PSYMBOL_INFO pSymInfo,
                                     _In_ ULONG SymbolSize,
                                     _In_opt_ PVOID UserContext)
{
  if (pSymInfo->Tag == 5 || pSymInfo->Tag == 10) {
...
const char*
MicroProfileDemangleSymbol(const char* pSymbol)
{
                                     ...
                                     uint32_t nNumSymbols)
{
  ...
  if (MicroProfileSymInit()) {
    ...
    for (uint32_t i = 0; i < nNumSymbols; ++i) {
      int nCount = snprintf(
        SymString, sizeof(SymString) - 1, "%s!%s", pModules[i], pSymbols[i]);
      if (nCount <= sizeof(SymString) - 1) {
        g_pFunctionFoundHack = 0;
        if (SymEnumSymbols(
              h, 0, SymString, MicroProfileQueryContextEnumSymbols1, 0)) {
          if (g_pFunctionFoundHack) {
            ...
            MicroProfileInstrumentFunction(
              g_pFunctionFoundHack, pModules[i], pDemangled, nColor);
          }
        }
      }
    }
...
static void*
MicroProfileAllocExecutableMemory(void* f, size_t s);
static void
MicroProfileMakeWriteable(void* p_);
  ...
  while (KERN_SUCCESS ==
    ...
    if (p >= (intptr_t)vmoffset && p <= intptr_t(vmoffset + vmsize)) {
      if (0 == (vbr.protection & VM_PROT_WRITE)) {
        ...
        int x =
          mprotect((void*)vmoffset, vmsize, PROT_WRITE | PROT_READ | PROT_EXEC);
        if (x) {
          // uprintf("mprotect failed ... err %d:: %d   %s\n", errno, x,
          // strerror(errno));
        } else {
          uprintf("region is [%llx,%llx] .. %08llx  %d",
                  vmoffset,
                  vmoffset + vmsize,
                  vmsize,
                  vbr.is_submap);
                           ...
                           uint32_t nNumModules)
{
  ...
  auto OnFunction = [&](void* addr,
                        void* addrend,
                        const char* pSymbol,
                        const char* pModuleName,
                        void* pModuleAddr) -> bool {
    const char* pStr = MicroProfileDemangleSymbol(pSymbol);
    ;
    int l =
      MicroProfileTrimFunctionName(pStr, &FunctionName[0], &FunctionName[1024]);
      ...
      if (0 != (vbr.protection & VM_PROT_EXECUTE)) {
        ...
        if (r) {
          uprintf("[0x%p-0x%p] (0x%p) %s %s\n",
                  ...
                  di.dli_sname);
          MicroProfileSymbolInitModule(
            di.dli_fname, (intptr_t)vmoffset, (intptr_t)vmoffset + vmsize);
        }
      }
    }
    vmoffset += vmsize;
...
extern "C" void
microprofile_tramp_enter() asm("_microprofile_tramp_enter");
extern "C" void
...
extern "C" void
microprofile_tramp_end() asm("_microprofile_tramp_end");
extern "C" void
microprofile_tramp_exit() asm("_microprofile_tramp_exit");
extern "C" void
microprofile_tramp_leave() asm("_microprofile_tramp_leave");
extern "C" void
microprofile_tramp_trunk() asm("_microprofile_tramp_trunk");
extern "C" void
...
MicroProfileSymbolUpdateModuleList()
{
  ...
  while ((read = getline(&line, &len, F)) != -1) {
    ...
    if (8 ==
        sscanf(
          line, "%p%c%p%c%c%c%c%c", &pBase, &c, &pEnd, &c, &r, &w, &x, &p)) {
      if ('x' == x) {
        int r = 0;
        r = dladdr(pBase, &di);
        if (r) {
          if ('[' != di.dli_fname[0]) {
            MicroProfileSymbolInitModule(
              di.dli_fname, (intptr_t)pBase, (intptr_t)pEnd);
          }
        }
      }
    }
...
MicroProfileAllocExecutableMemory(void* f, size_t s)
{
  ...
  void* pMem = mmap(
    f, s, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANON | MAP_PRIVATE, 0, 0);
  return pMem;
}
...
uint64_t
MicroProfileHashTableHashString(uint64_t pString)
{
  return MicroProfileStringHash((const char*)pString);
}

bool
MicroProfileHashTableCompareString(uint64_t L, uint64_t R)
{
...
void
MicroProfileStringBlockFree(MicroProfileStringBlock* pBlock)
{
...
MicroProfileStringBlock*
MicroProfileStringBlockAlloc(uint32_t nSize)
{
  nSize = MicroProfileMax(nSize,
                          (uint32_t)(MicroProfileStringBlock::DEFAULT_SIZE -
...
MicroProfileStringsInit(MicroProfileStrings* pStrings)
{
  MicroProfileHashTableInit(&pStrings->HashTable,
                            1,
                            25,
...
MicroProfileStringInternLower(const char* pStr)
{
  return MicroProfileStringIntern(
    pStr, (uint32_t)strlen(pStr), ESTRINGINTERN_LOWERCASE);
}

const char*
MicroProfileStringIntern(const char* pStr_, uint32_t nLen, uint32_t nFlags)
{
...
void
DumpTable(MicroProfileHashTable* pTable)
{
  for (uint32_t i = 0; i < pTable->nAllocated; ++i) {
...
void
DumpTableStr(MicroProfileHashTable* pTable)
{
  int c = 0;
  (void)c;
  for (uint32_t i = 0; i < pTable->nAllocated; ++i) {
...
#define NUM_ITEMS 100
  ...
  MicroProfileHashTableInit(&Strings,
                            1,
                            25,
                            MicroProfileHashTableCompareString,
                            MicroProfileHashTableHashString);
